...
import { jobsAPI, appsAPI, subsAPI } from '../api'
          ...
          {tab === 'overview' && (
            <div>
              <div style={{ display:'flex', alignItems:'center', gap:'1rem', marginBottom:'1.25rem' }}>
                ...
                <div style={{ marginLeft:'auto', fontFamily:'Space Grotesk,sans-serif', fontWeight:700, color:'var(--teal)' }}>
                  AED {(job.salMin/1000).toFixed(0)}K – {(job.salMax/1000).toFixed(0)}K
                </div>
              </div>
      ...
      <section id="hero" className="hero-section" style={{ position:'relative', minHeight:'100vh', display:'flex', alignItems:'center', padding:'120px 2rem 80px', overflow:'hidden' }}>
        ...
        <div className="hero-inner" style={{ position:'relative', zIndex:1, maxWidth:1200, margin:'0 auto', width:'100%' }}>
          <div style={{ display:'inline-flex', alignItems:'center', gap:'0.6rem', background:'rgba(58,123,255,0.1)', border:'1px solid var(--border)', borderRadius:100, padding:'0.45rem 1rem 0.45rem 0.7rem', fontFamily:'Space Grotesk,sans-serif', fontSize:'0.72rem', fontWeight:700, letterSpacing:'0.12em', textTransform:'uppercase', color:'var(--accent-pale)', marginBottom:'1.75rem' }}>
            ...
            We're Hiring · DIFC Dubai & Remote
          </div>
          <h1 style={{ fontSize:'clamp(2.6rem,5.5vw,4.8rem)', fontWeight:700, marginBottom:'1.5rem', lineHeight:1.08, maxWidth:700 }}>
            Build the <em style={{ fontStyle:'italic', color:'var(--accent)' }}>AI Future</em><br/>from the Heart<br/>of the Gulf
          </h1>
          <p style={{ fontSize:'1.1rem', color:'var(--muted)', marginBottom:'2.5rem', maxWidth:520, lineHeight:1.75 }}>
            Solvagence AI Consulting is the GCC's premier enterprise AI transformation firm — headquartered in DIFC Dubai, operating across 12+ countries.
          </p>
          <div style={{ display:'flex', gap:'1rem', flexWrap:'wrap', marginBottom:'3rem' }}>
            <a href="#open-roles" className="btn-primary">🔍 View Open Roles</a>
            <a href="#signup" className="btn-secondary">Get Career Alerts</a>
          </div>
          <div className="hero-stats" style={{ display:'flex', gap:'2.5rem', flexWrap:'wrap', paddingTop:'2.5rem', borderTop:'1px solid var(--border-s)' }}>
            {[['35+','Open Positions'],['12+','Countries Active'],['92%','Team Satisfaction'],['4×','YoY Growth']].map(([n,l]) => (
              <div key={l}>
                <div style={{ fontFamily:'Crimson Text,serif', fontSize:'2.2rem', fontWeight:700, lineHeight:1 }}>{n}</div>
                <div style={{ fontSize:'0.78rem', color:'var(--muted)', marginTop:'0.3rem' }}>{l}</div>
              </div>
            ))}
          </div>
        </div>
      ...
      <section id="open-roles" className="section" style={{ background:'var(--navy-mid)' }}>
        <div className="inner">
          <div className="section-head">
            ...
            <p className="section-desc">From frontier AI engineering to C-suite consulting — exceptional people, exceptional work.</p>
            ...
            {jobs.length ? jobs.map(j => (
              <div key={j._id} style={s.card} onClick={() => setSelJob(j)}
                ...
                <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', paddingTop:'1rem', borderTop:'1px solid var(--border-s)' }}>
                  <span style={{ fontFamily:'Space Grotesk,sans-serif', fontSize:'0.85rem', fontWeight:700, color:'var(--teal)' }}>AED {(j.salMin/1000).toFixed(0)}K – {(j.salMax/1000).toFixed(0)}K</span>
      ...
      <section id="why" className="section">
        <div className="inner">
          <div className="section-head c">
            ...
            <h2 className="section-title">Where <em>Excellence</em> Meets Purpose</h2>
            <p className="section-desc">We don't just offer jobs — we offer a mission.</p>
            <div className="divider c" />
          </div>
          <div className="feature-grid">
            {[['🌍','Global Impact, DIFC Prestige','Work from the heart of Dubai\'s premier financial district while shaping AI adoption across 12+ countries. Your work matters globally.'],['💸','Tax-Free, Top-Tier Pay','Benchmarked against McKinsey, Accenture, and FAANG. UAE offers 0% income tax. Packages include bonuses, equity, and comprehensive benefits.'],['🚀','Hypergrowth & Ownership','4× YoY growth. Every team member owns meaningful scope from day one. Direct access to Md Anwer and leadership.'],['🤖','Frontier AI Work','Engage with cutting-edge LLMs, custom model development, and enterprise AI deployments for the GCC\'s largest institutions.'],['🎓','AED 15K Learning Budget','Annual budget for certifications, conferences, and courses. Internal AI masterclasses, sponsored certs, and a culture of curiosity.'],['🤝','Elite Client Access','Advise sovereign wealth funds, Tier 1 banks, and government ministries. Build relationships that last a career.'],['🌴','UAE Lifestyle & Visa','Full visa sponsorship, Emirates ID, and relocation support. Thrive in one of the world\'s most dynamic cities.'],['🧬','Diverse & Inclusive','40+ nationalities. Gender-balanced leadership. Emiratisation is a core pillar. All backgrounds, one mission.']].map(([icon,title,desc]) => (
      ...
      <section id="benefits" className="section" style={{ background:'var(--navy-mid)' }}>
        <div className="inner">
          <div className="section-head">
            ...
            <h2 className="section-title">Rewarding <em>Excellence</em> at Every Level</h2>
            <p className="section-desc">Top-quartile packages benchmarked against the best global firms.</p>
            <div className="divider" />
          </div>
          <div className="benefits-grid">
            {[['💰','Competitive Salary + Bonus','Tax-free AED income benchmarked globally. 10–30% annual bonus + spot awards.'],['✈️','UAE Visa & Relocation','Full sponsorship, Emirates ID, one-way airfare, and temporary housing for international joiners.'],['🏥','Premium Health Insurance','DHA-compliant medical cover for you and dependents. Dental, optical, mental health — fully funded.'],['📚','AED 15,000 Learning Budget','Certifications, conferences, courses, books. 100% employer-sponsored. Partial rollover.'],['🏡','Hybrid Work Options','DIFC HQ base with structured hybrid. Remote-eligible for select roles. Core hours model.'],['📈','Stock Appreciation Rights','Equity-linked SARs for senior hires. 3-year vest, 1-year cliff. Direct upside as Solvagence scales.'],['🌴','28 Days Leave + Birthday Off','UAE Labour Law compliant. All public holidays + 2-week year-end company shutdown.'],['🧘','Wellbeing & Lifestyle','Monthly gym allowance, counselling access, quarterly team events, annual retreat.']].map(([icon,title,desc]) => (
      ...
      <section id="process" className="section">
        <div className="inner">
          <div className="section-head c">
            ...
            <h2 className="section-title">Transparent, <em>Respectful</em> & Fast</h2>
            <p className="section-desc">We respect your time. Most roles close in under 3 weeks from first application.</p>
            <div className="divider c" />
          </div>
          <div className="process-grid">
            {[['01','Apply Online','Submit your application with your CV in under 10 minutes. We read every application personally.'],['02','Talent Review','Our team reviews within 5 business days. No ghosting — you always hear back.'],['03','Intro Call','30-minute video call. Conversational, not interrogative. We want to hear your story.'],['04','HM Interview','60-minute deep dive with your future manager. Technical depth and culture fit.'],['05','Assessment','A focused, realistic problem — 2–4 hours max. No trick questions.'],['06','Offer in 48hrs','Final leadership conversation, then a competitive offer within 48 hours.']].map(([num,title,desc]) => (
      ...
      <section className="section" style={{ background:'var(--navy-mid)' }}>
        <div className="inner">
          <div className="section-head c">
            ...
            <h2 className="section-title">Hear from Our <em>People</em></h2>
            <div className="divider c" />
          </div>
          <div className="testimonials-grid">
            {[['MK','Meera K.','Senior AI Engineer · Dubai','I\'ve grown more in 18 months at Solvagence than in my previous five years combined. The clients are world-class, the team is exceptional.'],['AA','Abdullah A.','AI Strategy Consultant · Abu Dhabi','As a UAE national, Solvagence put me on C-suite AI projects within my first quarter. The opportunities and trust are very real.'],['ST','Dr. Sarah T.','AI Research Lead · Remote','Publishing papers AND shipping AI to production — simultaneously. Genuinely the best of both worlds.'],['JM','James M.','MLOps Engineer · Dubai','London to Dubai in 8 weeks — the visa and relocation process was seamless. The team made it easy.'],['PR','Priya R.','Data Science Director · Dubai','I\'ve worked at Big Tech and Big 4. Solvagence is different — lean enough to move fast, senior enough for the hardest problems.'],['KF','Khalid F.','Cloud Architect · Dubai','Three international conferences, two Azure certifications — the company actively pushed me to take every opportunity.']].map(([init,name,role,text]) => (
              <div key={name} style={{ background:'var(--navy-card)', border:'1px solid var(--border-s)', borderRadius:'var(--r-lg)', padding:'2rem', position:'relative', transition:'var(--tr)' }}
                onMouseEnter={e=>e.currentTarget.style.borderColor='var(--border)'} onMouseLeave={e=>e.currentTarget.style.borderColor='var(--border-s)'}>
                <div style={{ fontFamily:'Crimson Text,serif', fontSize:'5rem', color:'var(--accent)', opacity:0.2, position:'absolute', top:'-0.5rem', left:'1.5rem', lineHeight:1, pointerEvents:'none' }}>"</div>
                <p style={{ fontSize:'0.88rem', color:'var(--offwhite)', lineHeight:1.8, marginBottom:'1.5rem', position:'relative' }}>{text}</p>
                <div style={{ display:'flex', alignItems:'center', gap:'0.85rem' }}>
                  <div style={{ width:42, height:42, borderRadius:'50%', background:'linear-gradient(135deg,var(--accent-dim),var(--accent))', display:'flex', alignItems:'center', justifyContent:'center', fontFamily:'Space Grotesk,sans-serif', fontSize:'0.8rem', fontWeight:700, color:'#fff', flexShrink:0 }}>{init}</div>
                  <div><div style={{ fontFamily:'Space Grotesk,sans-serif', fontSize:'0.85rem', fontWeight:700 }}>{name}</div><div style={{ fontSize:'0.75rem', color:'var(--muted)' }}>{role}</div></div>
                </div>
              </div>
            ))}
          </div>
          ...
          <div className="signup-grid">
            <div>
              <div className="section-head">
                ...
                <h2 className="section-title">Get <em>Early Access</em> to New Roles</h2>
        ...
        <div className="inner">
          <div className="footer-grid">
            <div>
              ...
              <p style={{ fontSize:'0.83rem', color:'var(--muted)', lineHeight:1.75, maxWidth:280 }}>Enterprise AI transformation, headquartered in DIFC Dubai. Building the AI future across GCC, Middle East, India, and USA.</p>
